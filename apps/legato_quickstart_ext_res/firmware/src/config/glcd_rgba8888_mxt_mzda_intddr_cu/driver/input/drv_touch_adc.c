...
short xRawTouch[DRV_TOUCH_ADC_SAMPLE_POINTS] = { 153,
                                                 872,
                                                 873,
                                                 172 };

short yRawTouch[DRV_TOUCH_ADC_SAMPLE_POINTS] = { 255,
                                                 828,
                                                 241,
                                                 810 };

/* This function is called after period matches in Timer  (32-bit timer) */
void _InterruptSvcRoutine(uint32_t status, uintptr_t context)
{
    /* Detect position  */
...
SYS_MODULE_OBJ DRV_Touch_ADC_Initialize(void)
{
    ...
    samplePoints.touchCalUlx = 153;
    samplePoints.touchCalUly = 255;

    samplePoints.touchCalUrx = 873;
    samplePoints.touchCalUry = 241;

    samplePoints.touchCalLrx = 872;
    samplePoints.touchCalLry = 828;

    samplePoints.touchCalLlx = 172;
    samplePoints.touchCalLly = 810;

    DRV_Touch_ADC_CalibrationSet(&samplePoints);

  _CallbackRegister(_InterruptSvcRoutine, NULL);
    _Start();
...
short DRV_Touch_ADC_PositionDetect(void)
{
    ...
    switch ( pDrvInstance->state ) 
    {
        ...
        {            
            if ( !ADCHS_ChannelResultIsReady(ADCHS_CH12) )
            {
                break; 
            }
            if ( (uint16_t) 256 <
                (uint16_t) ADCHS_ChannelResultGet(ADCHS_CH12)>>2 )
            {
                adcX = -1;
                adcY = -1;
            } 
        ...
        case DRV_TOUCH_ADC_STATE_GET_X:
        {
            ...
            temp = (uint16_t) ADCHS_ChannelResultGet(ADCHS_CH12)>>2;
            if (temp != tempX) 
            {
                tempX = temp;
                pDrvInstance->state = DRV_TOUCH_ADC_STATE_RUN_X;
